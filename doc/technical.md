...
协议解析器只接受 choices/widget/skill_check/state/map_update/inventory/reputation/party_change/session_end 白名单命令。状态按 Cartridge 的 min/max 夹紧；未知命令不进入 UI 或存档。远程 Adapter 过滤 thinking，只在完整回合确认后提交状态。
