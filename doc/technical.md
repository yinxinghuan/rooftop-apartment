...
协议解析器只接受 choices/widget/skill_check/state/map_update/inventory/reputation/party_change/session_end 白名单命令，并兼容 AI 偶发缺少角色左方括号的台词格式。状态按 Cartridge 的 min/max 夹紧；未知命令不进入 UI 或存档。Aigram Adapter 每轮携带权威状态与最近 20 个非图片剧情块；远程 Adapter 过滤 thinking。两者都只在完整回合确认后提交。
