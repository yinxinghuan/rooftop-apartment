...
- 世界实体字段位于 `types.ts`，协议由 `protocol.ts` 解析、`reducer.ts` 原子写入；`useStoryEngine.requestItemImage()` 把物品加入与剧情图共用的串行 worker，状态与 URL 保存在 `InventoryItem`。题材内容统一在 `rooftopApartment.ts` 配置。
