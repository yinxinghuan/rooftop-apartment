...
场景出图采用 AI 提议时机、本地 `imageDirector` 构建内容的结构。Aigram 与 remote Adapter 都会提取 `[image_prompt]`，但最终 prompt 以当前地点和本轮可见事件重建，避免屋顶公寓开场构图反复渗入后续场景。AI 未提议时，`engine/imageDirector.ts` 按首次进入空间、稀有物品、住户变化和阶段节点强制补图，关系/目标/检定变化使用 2 回合冷却，连续 4 个有效回合无图时按可见结果兜底。开场 prompt 只用于 scene 0；`MapNode.visited` 区分首次进入与回访；每轮最多追加一个带 `source/reason` 的内联图片块，仍进入严格串行 worker。
